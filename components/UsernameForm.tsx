      ...
      <FormProvider {...(form as any)}>
        <Card className="w-full max-w-md bg-card/90 backdrop-blur-xl border border-border shadow-xl rounded-3xl overflow-hidden mt-8 md:mt-0 p-8">
          <div className="flex flex-col w-full items-center justify-center pb-6">
            ...
            <h1 className="font-bold text-2xl text-foreground tracking-tight">Choose a Username</h1>
            <p className="text-muted-foreground text-sm mt-2 text-center">Pick a unique handle to get started</p>
          </div>
          <Form onSubmit={handleSubmit} method="POST">
            ...
            <Button
              type="submit"
              className="w-full h-12 rounded-xl bg-primary hover:bg-primary/90 text-white font-semibold transition-all shadow-sm mt-6"
            >
              Continue
            </Button>
          </Form>
